    ...
    namespace grpc_json_client
    {
        ...
        */
        ...
        TEST_F(GrpcJsonClientTest, WriteSucceedsOnValidWrite)
        {
            json request = { {"session_name", "gtest"}, {"resource_name", "VST2_01"} };
            void* tag1;
            ASSERT_EQ(StartAsyncCall(session, "nirfsa_grpc.NiRFSA", "Init", request.dump().c_str(), &tag1), 0);

            int timeout = 10000;
            size_t size = 0;
            ASSERT_EQ(FinishAsyncCall(session, tag1, timeout, nullptr, &size), 0);
            unique_ptr<char> buffer(new char[size]);
            ASSERT_EQ(FinishAsyncCall(session, tag1, timeout, buffer.get(), &size), 0);
            json response = json::parse(buffer.get());
            ASSERT_EQ(response["status"], 0);
            json vi = response["vi"];
            ...
            ASSERT_EQ(StartAsyncCall(session, "nirfsa_grpc.NiRFSA", "SetAttributeViReal64", request.dump().c_str(), &tag1), 0);
            
            request.clear();
            request["vi"] = vi;
            void* tag2;
            ASSERT_EQ(StartAsyncCall(session, "nirfsa_grpc.NiRFSA", "Commit", request.dump().c_str(), &tag2), 0);
            ...
            ASSERT_EQ(FinishAsyncCall(session, tag2, timeout, nullptr, &size), 0);
            buffer = unique_ptr<char>(new char[size]);
            ASSERT_EQ(FinishAsyncCall(session, tag2, timeout, buffer.get(), &size), 0);
            response = json::parse(buffer.get());
            ASSERT_EQ(response["status"], 0);
        }
    }
