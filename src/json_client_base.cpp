...
void JsonClientBase::QueryReflectionService() {
    ...
    unique_ptr<ClientReaderWriter<ServerReflectionRequest, ServerReflectionResponse>> stream = {
        reflection_stub.ServerReflectionInfo(&context)
    };
    ...
    for (ServiceResponse service : services) {
        ...
        const FileDescriptorResponse& file_descriptor_response = {
    ...
    if (!status.ok()) {
        string summary = {
            "Failed to retreive file descriptors from the host. "
